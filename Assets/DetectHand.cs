...
public class DetectHand : MonoBehaviour 
{
    ...
    void Detect(Mat rgbaMat)
    {
        ...
        var stats = new Mat();
        var nLabels = connectedComponentsWithStats(_handMask, new Mat(), stats, centroids);
        ...
        var maxAreaCentroid = new Point(centroids.get(maxAreaIndex, 0)[0], centroids.get(maxAreaIndex, 1)[0]);
        print(maxAreaCentroid);
        
        cvtColor(_handMask, _rgbMat, COLOR_GRAY2RGB);
        circle(_rgbMat, maxAreaCentroid, 30, new Scalar(255, 0, 0), -1);
